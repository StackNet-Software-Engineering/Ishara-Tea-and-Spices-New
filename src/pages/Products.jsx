...
function Products({ lang = 'en' }) {
  ...
  const totalPages = Math.ceil(currentCategory.items.length / ITEMS_PER_PAGE);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#fdfaf5]">
      ...
      <div className="relative z-10 w-full">
        ...
        {/* CATEGORY FILTER */}
        <div className="flex justify-center gap-3 md:gap-5 mb-10 flex-wrap px-4 mt-8">
          {categories.map((cat) => (
            <button
              key={cat.id}
              ...
              style={
                activeCategory === cat.id
                  ? { backgroundColor: '#1B4332', color: '#ffffff', border: '2px solid #1B4332', boxShadow: '0 4px 14px rgba(27,67,50,0.25)', transform: 'scale(1.05)' }
                  : { backgroundColor: 'transparent', color: '#3a5a47', border: '2px solid rgba(27,67,50,0.3)', }
              }
            >
        ...
        {/* Page count */}
        <div className="text-center mb-6 text-xs font-semibold tracking-widest uppercase" style={{ color: '#8a7a60' }}>
          {t.showing} {(currentPage - 1) * ITEMS_PER_PAGE + 1}–{Math.min(currentPage * ITEMS_PER_PAGE, currentCategory.items.length)} {t.of} {currentCategory.items.length} {t.products}
        </div>
          ...
          >
            {paginatedItems.map((item) => (
              <div
                ...
                onMouseEnter={e => e.currentTarget.style.boxShadow = '0 8px 32px rgba(27,67,50,0.22)'}
                onMouseLeave={e => e.currentTarget.style.boxShadow = '0 2px 12px rgba(27,67,50,0.10)'}
              ...
              >
                ...
                <div
                  ...
                  style={{ background: 'linear-gradient(to top, rgba(11,31,21,0.85) 0%, rgba(27,67,50,0.12) 50%, transparent 100%)' }}
                />
                {/* Hover overlay */}
                <div
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mb-20 flex-wrap px-4">
            <button
              ...
              style={{ border: '2px solid rgba(27,67,50,0.3)', color: '#2d5a3d', backgroundColor: 'transparent' }}
              onMouseEnter={e => { if (currentPage !== 1) { e.currentTarget.style.backgroundColor = 'rgba(27,67,50,0.08)'; e.currentTarget.style.borderColor = '#1B4332'; }}}
              onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.borderColor = 'rgba(27,67,50,0.3)'; }}
            >
              {t.prev}
            </button>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => {
              ...
              if (showEllipsisBefore || showEllipsisAfter)
                return <span key={`e-${page}`} style={{ color: '#a09070' }} className="px-1">…</span>;
              ...
              return (
                <button
                  ...
                  style={
                    currentPage === page
                      ? { backgroundColor: '#1B4332', color: '#ffffff', border: '2px solid #1B4332', transform: 'scale(1.1)', boxShadow: '0 2px 10px rgba(27,67,50,0.3)' }
                      : { border: '2px solid rgba(27,67,50,0.25)', color: '#2d5a3d', backgroundColor: 'transparent' }
                  }
                >
                  {page}
                </button>
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-full font-semibold transition duration-200 text-sm disabled:opacity-30 disabled:cursor-not-allowed"
              style={{ border: '2px solid rgba(27,67,50,0.3)', color: '#2d5a3d', backgroundColor: 'transparent' }}
              onMouseEnter={e => { if (currentPage !== totalPages) { e.currentTarget.style.backgroundColor = 'rgba(27,67,50,0.08)'; e.currentTarget.style.borderColor = '#1B4332'; }}}
              onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.borderColor = 'rgba(27,67,50,0.3)'; }}
            >
              {t.next}
            </button>
